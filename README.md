...
Это pet-проект, демонстрирующий полный цикл ML-разработки:
от детекции объектов и OCR до интеграции с LLM API и десктопного UI.
Продолжаю дорабатывать по мере возможностей.

**Платформа:** Windows 10+ (протестировано на Windows 10). 
Поддержка мультимониторности, DPI Awareness, прозрачный оверлей и глобальные горячие клавиши.
...
- OCR (PaddleOCR с fallback на адаптивный порог при пустом результате).
...
3. Облачки обрезаются с небольшими полями (padding), чтобы не захватить
   границы пузыря; если OCR на обрезке вернул пустоту, распознавание
   повторяется на всей области с предварительным адаптивным порогом.
...
| **Q** / **Й** | Запустить перевод |
| **ЛКМ** | Скрыть перевод (клик по оверлею) |
...
Отредактируйте `.env`, указав `API_KEY`. Также параметры
...
- **Стилизованные шрифты.** PaddleOCR может ошибаться на декоративных
  шрифтах и звуковых эффектах (SFX).

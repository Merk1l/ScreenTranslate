...
Пользователь выделяет область на экране, приложение находит speech bubble,
распознаёт английский текст и показывает русский перевод поверх изображения
в виде прозрачного оверлея.

Разрабатывается как pet-проект и демонстрация full-stack ML-разработки:
...
Приложение работает с любым OpenAI-совместимым API. В UI есть готовые пресеты:

| Провайдер | Назначение |
|-----------|-----------|
| **Groq** | Быстрый и дешёвый инференс (llama-3.3, gpt-oss) |
| **OpenAI** | GPT-4o / GPT-4o-mini |
| **Ollama** | Полностью локальный инференс |
| **LocalAI** | Локальный сервер |
...
```
ScreenTranslate/
  main.py              # все приложение в одном модуле
